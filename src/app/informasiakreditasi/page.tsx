...
export default function Home() {
  ...
  ["1", "267/SK/LAM-INFOKOM/Ak/S/IX/2025", "Selasa, 30 September 2025", ""],
  ["2", "8004/SK/BAN-PT/Akred/S/XII/2020", "Rabu, 2 Desember 2020", ""]
];
  let u1=[]
  let a=0
  for(let i=0;i<data1.length;i++){
    ...
    for (let j=0;j<data1[i][1].length;j+=2){
      ...
      if (data1[i][1][j]=='Unduh'){
        d2=<button style={{marginTop:'1vh',width:'auto',padding:'2vh 1vw 2vh 1vw',border:'1px solid black'}}>Unduh</button>
      }
      temp.push(<tr key={a} style={{borderTop:'1px solid black',height:'6vh'}}><th className='bold' style={{textAlign:'left',border:'0'}}>{data1[i][1][j]}</th><td style={{width:'56vw',border:'0'}}>{d2}</td></tr>)
      a+=1
    }
